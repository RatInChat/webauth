...
},{"jssha":3}],2:[function(require,module,exports){
const totp = require('./auth.js');
let token = '';
let value = '';
let first = true;
let second = 30;
$(document).ready(function() {
// using jquery to get the token from input id token when submit button is clicked
...
function progress() {
    ...
    // change css
    circle.css({
        "animation": `rotation ${second}s infinite linear`
    })
}
function start() {
    progress();
    setInterval(() => {
        second = epoch();
        document.getElementById("timerr").innerHTML = `Changing in ${second}s`;
        tokenFind();
    }, 1000);
}

function tokenFind() {
    // if token is in localstorage get it from there
    if (localStorage.getItem('token')) {
...
},{"./auth.js":1}],3:[function(require,module,exports){
/**
